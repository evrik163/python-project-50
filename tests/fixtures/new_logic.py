...
def plain_formatter(lst, path=''):
    ...
    for i in lst:
        ...
        if i['status'] == "deleted":
            result.append(f"Property '{prop}' was added with value: {value_check(i['value'])}")
        if i['status'] == "changed":
            result.append(f"Property '{prop}' was updated. From {value_check(i['old_value'])} to {value_check(i['new_value'])}")
        if i['status'] == "nested":
            result.append(plain_formatter(i['value'], f"{prop}."))
    return '\n'.join(result)

print(plain_formatter(result))
